...
@section('content')
    ...
    <section class="content">
        <form method='post' enctype="multipart/form-data" action="{{ isset($item) ? route('slider.update', $item->id) : route('slider.store') }}">
            ...
            <div class="col-md-8">
                <div class="form-group">
                    <h3>
                        @isset($item)
                            {{ __('main.edit') }} {{ __('main.slider') }}
                            <a href="{{ route('slider.create') }}" class="btn btn-outline-primary btn-xs">{{ __('main.add_new') }}</a>
                        @else
                            {{ __('main.add') }} {{ __('main.slider') }}
                        @endisset
                    </h3>
