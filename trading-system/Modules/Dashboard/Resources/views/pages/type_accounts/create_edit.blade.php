...
@section('content')
    ...
    <section class="content">
        <form method='post' enctype="multipart/form-data" action="{{ isset($item) ? route('typeAccounts.update', $item->id) : route('typeAccounts.store') }}">
            ...
            <div class="col-md-8">
                <div class="form-group">
                    <h3>
                        @isset($item)
                            {{ __('main.edit') }} {{ __('main.Type_Account') }}
                            <a href="{{ route('typeAccounts.create') }}" class="btn btn-outline-primary btn-xs">{{ __('main.add_new') }}</a>
                        @else
                            {{ __('main.add') }} {{ __('main.Type_Account') }}
                        @endisset
                    </h3>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group {{ $errors->has('content') ? ' has-error' : '' }}">
                                <label class="control-label">{{ __('main.content') }}</label>
