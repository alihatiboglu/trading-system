...
@section('content')
    ...
    <section class="content">
        <form method='post' enctype="multipart/form-data" action="{{ isset($item) ? route('ads.update', $item->id) : route('ads.store') }}">
            ...
            <div class="col-md-8">
                <div class="form-group">
                    <h3>
                        @isset($item)
                            {{ __('main.edit_ads') }}
                            <a href="{{ route('ads.create') }}" class="btn btn-outline-primary btn-xs">{{ __('main.add_new') }}</a>
                        @else
                            {{ __('main.add_adds') }}
                        @endisset
                    </h3>
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group {{ $errors->has('url') ? ' has-error' : '' }}">
                                <label class="control-label">{{ __('main.url') }}</label>
