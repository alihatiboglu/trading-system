...
@section('content')
    ...
    <section class="content">
        <form method='post' enctype="multipart/form-data" action="{{ isset($item) ? route('packages.update', $item->id) : route('packages.store') }}">
            ...
            <div class="col-md-8">
                <div class="form-group">
                    <h3>
                        @isset($item)
                            {{ __('main.edit') }} {{ __('main.wallet') }}
                            <a href="{{ route('packages.create') }}" class="btn btn-outline-primary btn-xs">{{ __('main.add_new') }}</a>
                        @else
                            {{ __('main.add') }} {{ __('main.wallet') }}
                        @endisset
                    </h3>
